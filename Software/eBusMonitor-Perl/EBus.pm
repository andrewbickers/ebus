package EBus;
use strict;

sub new {
	my ($class) = @_;
	my $self = {};
	return bless $self, $class;
}

my @crcs = (
	
);

sub crc8 {
	my ( $data, $init ) = @_;
	my $crc;
	$crc = $crcs[$init] ^ ord($data);
	return $crc;
}

sub calcCrc {
	my ( $self, $str ) = @_;
	my $len = length($str);
	my $crc = 0;
	for ( my $i = 0 ; $i < $len ; $i++ ) {
		$crc = crc8( substr( $str, $i, 1 ), $crc );
	}
	return $crc;
}

sub calcCrcExpanded {
	my ( $self, $str ) = @_;
	my $tmp = "";
	for ( my $i = 0 ; $i < length($str) ; $i++ ) {
		my $chr = substr( $str, $i, 1 );
		if ( ord($chr) == 170 ) {
			$tmp .= chr(0xA9) . chr(0x01);
		}
		else {
			$tmp .= $chr;
		}
	}
	my $len = length($tmp);
	my $crc = 0;
	for ( my $i = 0 ; $i < $len ; $i++ ) {
		$crc = crc8( substr( $tmp, $i, 1 ), $crc );
	}
	return $crc;
}

sub bcd {
	my ( $self, $byte ) = @_;
	my $high = $byte >> 4;
	my $low  = ( $byte & 15 );
	return $high * 10 + $low;
}

sub data1b {
	my ( $self, $byte ) = @_;
	if (($byte & 0x80) == 0x80) {
		return -((~$byte & 255) + 1);
	}
	else {
		return $byte;
	}
}

sub data1c {
	my ( $self, $byte ) = @_;
	return $byte/2;
}

sub data2b {
	my ( $self, $lByte, $hByte ) = @_;
	my $word = $hByte*256 + $lByte;
	if (($word & 0x8000) == 0x8000) {
		return -((~$hByte & 255) + ((~$lByte & 255) + 1)/256)
	}
	else {
		return $hByte + $lByte/256
	}
}

sub data2c {
	my ( $self, $lByte, $hByte ) = @_;
	my $word = $hByte*256 + $lByte;
	if (($word & 0x8000) == 0x8000) {
		 return -((~$hByte & 255)*16 + ((~$lByte & 255) >> 4 ) + ((((~$lByte & 255) & 15)+1)/16));
	}
	else {
		return $hByte*16 + ($lByte >> 4 ) + ($lByte & 15)/16;
	}
}



1;
